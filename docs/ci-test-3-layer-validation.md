...
- 서비스별 테스트 범위가 불균등했고, 대부분의 서비스가 테스트 0건 / CI 미연동 상태였음
...
| 서비스 | 테스트 파일 | 테스트 메서드 | CI 실행 |
|---|---|---|---|
| workpay-service | 44개 | 358개 | O (14개 패턴 + 계약 테스트) |
...
### 테스트 케이스 카탈로그 (서비스 × 도메인별)

> 51개 파일 / 410개 테스트 메서드를 도메인별로 정리. 각 도메인이 어떤 케이스를 검증하는지 키워드로 요약.

<details>
<summary><b>workpay-service (358 메서드) — 클릭하여 펼치기</b></summary>

#### 금액 / 시급 변환
- 시급 × 근무시간 계산, 두 금액 합산, 가산율(50%) 적용
- 3.3% 원천세 차감 / 세금 미적용 분기
- null 입력 → ZERO 반환 (NPE 차단)
- 시급제: 금액 그대로 통상시급
- 일급제: 일급 ÷ 근무시간 → 통상시급 환산 (근무시간 미설정 시 8시간 기본값)
- 월급제: 월급 ÷ 209시간 → 통상시급 환산 (미설정 시 209시간 기본값)
- 동등성 / hashCode 일관성
...
#### 가산수당 분기 (연장/야간/휴일/주휴)
- 09~ 18시 평일 주간 + 모든 수당 OFF → 가산수당 0
- 야간수당 ON + 22시 이후 근무 → 50% 가산
- 야간수당 OFF → 22시 이후도 0
- 일요일/공휴일 + 휴일수당 ON → 휴일 가산율
- 평일 + 휴일수당 ON → 가산 없음
- 일 8시간 초과 + 연장수당 ON → 연장 가산
- 일 8시간 이하 + 연장수당 ON → 가산 없음 (조건 미충족)
- 일요일 야간 연장 → 모든 가산수당 누적

#### 야간 시간 분리 (22:00~ 06:00) — 경계값 33가지
- 자정 안 넘김: 22:00~  23:30, 17:00~  23:00, 14:00~  23:00, 09:00~  23:59, 21:00~  23:00 등 시작·종료가 22~  06 구간 안팎에 걸치는 모든 패턴
- 자정 넘김: 22:00~  02:00, 20:00~  03:00, 22:00~  06:00, 23:00~  05:00, 20:00~  08:00 등 0시 통과 케이스
- 새벽 단독: 04:00~  06:00, 04:00~  10:00, 05:00~  08:00 등 06시 경계
- 경계값 확인: 06:00~  22:01 → 1분, 18:00~  22:00 → 0분 (22:00 도달했지만 미포함)
- 이른 아침: 00:00~  06:00 → 360분, 05:00~  05:30 → 30분
- start == end (22:00~  22:00) → 0분
- null 입력 안전성 (NPE 차단)

#### 주간 시간 분리 (06:00~ 22:00) — 경계값 10가지
- 09:00~  18:00 → 540분, 06:00~  22:00 → 960분 (전체 주간)
- 22:00~  06:00 / 23:00~  05:00 / 00:00~  06:00 → 0분 (전체 야간)
- 자정 넘김 20:00~  08:00 → 주간 240분 (20~ 22 + 06~ 08)
- 04:00~  10:00 → 주간 240분 (06~  10)
- 18:00~  00:00 → 주간 240분 (18~  22)
- 경계값 05:00~  07:00 → 60분 (06~  07)

#### 휴게시간 자동 배치
- 09~ 18시 + 60분 주간 휴게 → 09~ 10시 배치
- 22~ 23:30 + 30분 야간 휴게 → 22~ 22:30 배치
- 자정 넘김 20~ 04 + 60분 휴게 → 첫 구간(20~ 22)에 배치
- 22~ 06 야간 전체 + 60분 휴게 → 야간 구간 안에 배치
- 휴게 0분 → 빈 리스트

#### 공휴일 / 최저시급
- 전체 공휴일 목록 조회, 이름 조회 (존재/없음 → null)
- 빈 공휴일 목록 처리
- 연도별 최저시급 조회, 데이터 없으면 기본값(10,030원) fallback
- 날짜 기반 조회, 날짜 null → 현재 년도 fallback
- 신규 저장 / 업데이트 / 삭제 (없는 년도 삭제 시 idempotent)

#### 시급 결정 / 히스토리 ★
- 신규 시급 등록 시 기존 시급의 endDate를 (새 시작일 - 1일)로 설정 → 날짜 갭 없음
- 최초 등록 시 기존 user_branch 시급을 히스토리로 보존
- 여러 번 변경 시 연속 날짜 체인 유지
- 미래 시작일 시급이 현재 적용 시급으로 표시 (날짜 계산)
- 정렬 안 된 리스트에서도 올바르게 선택
- **fallback 분기 4가지**
  - 케이스1: salary_history 존재 → 해당 시급 사용
  - 케이스2: history 없음 + personalCost 있음 → personalCost 사용 (silent 최저시급 차단)
  - 케이스3: history 없음 + personalCost null → 최저시급 fallback (NPE 차단)
  - 케이스4: history 없음 + personalCost 0원 → 최저시급 fallback (0원 silent 계산 차단)
- 정책 삭제 시 존재하지 않으면 예외 / 브랜치별 정책 그룹 조회

#### 4대보험 / 공제 계산
- 국민연금/건강보험/고용보험/산재보험 각각 신규 저장 + 업데이트
- 지점별 보험 금액 맵 조회
- 캐시 히트/미스 동작 (Caffeine put)
- 0원 / 음수 입력 → 공제 0
- 3.3% 세금 적용 / OFF 분기
- deductions null → 원금 그대로 (방어 코드)
- 4대보험 전체 적용 / 국민연금만 / 세금+4대보험 동시
- insurance null → 보험 공제 없음
- 보험 금액 null → 0으로 처리

#### 사용자 설정 (페이지/공제/수당/보험)
- 신규 / 부분 업데이트 / 조회 (기본값) / 삭제
- 정렬 옵션 SALARY_DESC / NAME_ASC
- 공제·수당·보험 설정 조회 (존재/null) + Map 조회
- 국민연금 금액 단독 업데이트

#### 급여 계산 통합
- 일별 급여 처리: UPSERT, 야간수당 포함, 휴게시간 차감, 일급/월급제 분기
- 급여 상세 조회: 데이터 없으면 null
- 연장수당 ON + 8시간 초과 → 금액 표시
- 연장수당 ON + 8시간 이하 → 미적용 사유 `DAILY_LIMIT_NOT_EXCEEDED`
- 연장수당 OFF → 미적용 사유 `SETTING_DISABLED`
- 세금/4대보험 전부 ON → 전부 공제 / 전부 OFF → 공제 없음
- 배치 프리로드 / 클리어

#### 주휴수당
- 주 15시간 이상 → 발생
- 주 15시간 미만 → 0원
- 정확히 15시간(900분) → 발생 (경계 포함)
- 주 40시간 초과 → 40시간 캡 적용
- 주휴수당 설정 OFF → 0원
- 시급 10,030원 + 주 25시간 → 실제 금액 검증

#### 급여 확정 (Confirmation)
- 일괄 확정: 유저 목록 비어있음/null → IllegalArgumentException
- 일괄 확정: 이미 전체 확정된 기간 → 즉시 success (idempotent)
- 일괄 확정: 개별 확정 안 된 직원 존재 → failure
- 개별 확정: 필수 필드 null → IllegalArgumentException
- 개별 확정: 급여 데이터 없음 → 스킵 (예외 없음)
- 개별 확정: 급여 데이터 있음 → 확정 + 알림 발행
- 취소: 데이터 있으면 처리, 없으면 스킵
- 상태 체크: 개별 확정 / 전체 확정 / 둘 다 / 둘 다 없음 케이스 분리
- userId null → 브랜치 전체 확정 체크 / branchId/workDate null → false
- Map 기반 조회: 개별/bulk/missing key 분기, 전체/빈 리스트/일부 미확정 분기

#### 배치 / 성능
- 배치 조회 vs 개별 조회 성능 비교 (50명 / 100명)
- 캐시 적용 후 성능 시뮬레이션
- 직원 수에 따른 스케일링

#### DTO 교차 필드 검증
- HOURLY: hourlyWage만 있으면 성공, dailyWage 있으면 실패, hourlyWage null이면 실패
- DAILY: dailyWage만 성공, hourlyWage 있으면 실패
- MONTHLY: monthlyWage만 성공, dailyWage 있으면 실패
- payType null → 다른 검증에서 처리 (위임)

#### 직원 앱 — 급여 조회 권한 (E2E)
- visibility=2 (항상 공개) → 직원 조회 가능
- visibility=1 + 미확정 → 차단
- visibility=1 + 확정완료 → 조회 가능
- visibility=0 (비공개) → 차단
- 사장이 앱에서 조회 → Manezy 웹 안내
- 존재하지 않는 유저 → 404
...
- 직원 개별 급여 확정
- 전체 급여 확정
- 권한 분기: 관리자(1) → 요약 조회 불가, 직원(0) → 확정 불가

#### 스케줄 (Calendar)
- 정상 생성 / 중복 시 ScheduleConflictException
- 조회: 존재하면 반환, 없으면 ResourceNotFoundException, 월별 조회
- 수정: 급여 확정된 스케줄은 수정 불가, 없는 스케줄 수정 시 404
- 삭제: 정상, 없으면 404, 급여 확정 시 삭제 불가
- 권한: 본인 true / 타인 false
- 존재 여부: 있음 true, 없음 false, 지점 없음 false, 예외 시 graceful false
- 충돌 검사: 있음/없음
- 배치 삭제 건수 반환 / 0건 처리

#### 스케줄 신청 (요청/승인/거절)
- 충돌 없음 → PENDING 저장
- 단건/반복 시간 충돌 → ScheduleConflictException
- 단건 승인 → createSchedule 호출
- DAILY 반복 승인 → createRepeatSchedules
- CUSTOM 반복 승인 → 요일 정보 포함된 RepeatScheduleRequest 생성
- PENDING 아닌 요청 승인/거절 불가
- PENDING 거절 시 사유 저장
- 조회: 직원별 / 지점별 (status 필터 있음/없음)

#### 퇴직 데이터 정리
- 퇴직일 이후 데이터만 삭제 (날짜 기반)
- quitDate null → 전체 삭제 fallback
- 삭제 중 예외 → 트랜잭션 롤백 (부분 삭제 없음)
- 데이터 없는 유저 → 예외 없이 처리 (멱등성)

#### Feign DTO 계약 (서비스 간 호환성)
- branch `BranchInfoDTO` → calendar/salary 양쪽 DTO 전체 필드 매핑
- 미지의 필드 추가 시 크래시 안 남 (`@JsonIgnoreProperties` 검증)
- branch `WorkerDTO` → salary DTO 핵심 필드
- user `UserInfoDTO` → calendar/salary DTO 전체 필드
- 배치 응답 (List) 역직렬화
- user / branch `UserWageSettingsDTO` 양쪽 응답 역직렬화
- 배치 응답 `Map<Long, DTO>` 역직렬화
- Provider가 안 보내는 필드 → null 정상 처리

</details>

<details>
<summary><b>branch-service (45 메서드) — 클릭하여 펼치기</b></summary>

#### 직원 등록 / 급여 설정
- 시급/일급/월급제 직원 추가 - 정상 케이스
- 시급제 추가 실패: personalCost null
- 일급제 추가 실패: dailyCost null
- 월급제 추가 실패: monthlyCost null / workMinutes null
- 추가 실패: workType null / 이미 존재하는 유저
- 급여 설정 조회: 시급/일급/월급제 정상
- 조회 실패: 존재하지 않는 유저 / 브랜치 / UserBranch 관계 없음
- 시급 우선순위 (시급과 일급이 동시에 있을 때 시급 우선)

#### 퇴사 / 복직
- 퇴사 처리: quitDate 저장 + outbox PENDING 이벤트 생성
- 퇴사 실패: quitDate 없으면 에러
- 원자성: quitDate 없으면 outbox도 생성 안 됨
- 복직 처리: quitDate null로 초기화, outbox 생성 안 됨
...
- 급여 설정 조회 확인
- 직원 삭제
- 권한 분기: 관리자(1) → 직원 추가 불가, 직원(0) → 급여 설정 변경/직원 삭제 불가

#### Feign DTO 계약 (billing → branch 방향)
- billing `CouponResponseDTO` → branch `CouponServiceClient.CouponResponseDTO` 전체 필드 매핑
- 미지의 필드 추가 시 크래시 안 남

</details>

<details>
<summary><b>user-service (7 메서드) — 클릭하여 펼치기</b></summary>

#### 인증 / 인가 (E2E)
- 이메일 회원가입
- 이메일/비밀번호 로그인 → JWT + branchAdmins 발급
- 로그인 후 프로필 조회
- 비밀번호 틀림 → 로그인 실패
- 가입되지 않은 이메일 → 로그인 실패
...
GitHub Actions, Gradle, JUnit 5, Jakarta Bean Validation, Jackson ObjectMapper
